...
<table><form method='post' action='accionModificarJuego'>
                <%
            if (ds!=null)
            {
            if (ds.getUsuario().getRol().equalsIgnoreCase("administrador"))
    ...
    Juego j = Persistencia.obtenerJuego(id);
    if (j!=null)
    {
                %>
                <tr>
                    <td> IDJuego: </td>
                    <td><%=j.getIDJuego()%><input type='hidden' name='id' value='<%=j.getIDJuego()%>'></td></tr>
                    <tr><td> Nombre: </td><td><input type='text' name='nombre' value='<%=j.getNombre()%>'></td></tr>
                    <tr><td> Precio: </td><td><input type='text' name='precio' value='<%=j.getPrecio()%>'></td></tr>
                    <tr><td> Fecha de Lanzamiento: </td><td><input type='text' name='fechalan' value='<%=j.getFechaLanzamiento()%>'> </td></tr>
                    <tr><td> Fabricante </td><td><input type='text' name='fabricante' value='<%=j.getFabricante()%>'></td></tr>
                <tr><td> Genero </td>
    ...
    %>
                ...
                <tr><td> ¿Juego Online? </td><td><input type='checkbox' name='juegoonline' checked='<%=j.isJuegoOnline()%>'> </td></tr>
                <tr><td> Ruta de la imagen del juego. </td><td><input type='text' name='imagenjuego' value='<%=j.getImagen()%>'> </td><td><img src='<%=j.getImagen()%>'></td></tr>
                <tr><td>Descripci&oacute;n del juego </td><td><textarea name='descripcion'><%=j.getDescripcion()%></textarea></td></tr>
                
                <tr><td>Plataformas: </td></tr>
                <%
                 ...
                 if (jpl!=null)
                 {
                    ...
                    Iterator<Plataforma> ip = pl.iterator();
                    Plataforma p;
                    while (ip.hasNext())
                    {
                        p = ip.next();
                %>
                            <tr><td><input type='checkbox' name="plataforma[]" value="<%=p.getIDPlataforma() %>" /><%=p.getDescripcion()%></td></tr>
<%
                    }
                } else{
                    juegosPlataforma jp;
